...
Private Sub Form_Load()
    ...
    iConstantVariable = 30 'Values from "0 to 200" is recommended
    MaxHeight = 40
    MaxLength = 40
    MinHeight = 3
    MinLength = 4
...
Private Sub Form_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If Button = vbLeftButton Then
        ...
        Green = Int(Rnd * 255)
        Blue = Int(Rnd * 255)
        Line (X, Y)-(X, Y + 100), RGB(Red, Green, Blue)
        Line (X, Y)-(X, Y - 100), RGB(Red, Green, Blue)
        Line (X, Y)-(X + 100, Y), RGB(Red, Green, Blue)
        Line (X, Y)-(X - 100, Y), RGB(Red, Green, Blue)
        For radius = 100 To 1 Step -1
        If radius > 0 Then
            Red = Int(Rnd * 255)
            Green = Int(Rnd * 255)
            Blue = Int(Rnd * 255)
