 ...
 */
public class FeatureEnv3 extends FeatureEnv 
{
    //=== FIELDS ===
    // see "FeatureEnv.java" for more information.
    
    //=== CONSTRUCTORS ===
    public FeatureEnv3(final Environment2D environment) {
        super(environment);
        setFeatureName("   ind-M");
    }
    
    //=== METHODS ===
    @Override
    public double f(final EnvCell2D state) {
        if (state.getCellTypeEnum() == EnvCell2D.CellType.MINE) {
            return 1;
        } else {
            return 0;
        }
